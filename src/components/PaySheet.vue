  ...
  <div class="wrap" @keydown.esc="dismiss">
    ...
    <div class="sheet" role="dialog" aria-modal="true" aria-labelledby="pay-title">
      ...
      <h2 id="pay-title" class="title">
        {{ credits && credits.total === 0 ? "You've used your free plans" : 'Top up' }}
      </h2>

      <p class="body muted">
        You're paying for the AI that writes the plan — not a subscription. No card, no account, no
        renewal.
      </p>
      ...
      <template v-if="price">
        <div class="quote">
          ...
          <p class="quote__for">
            buys <strong>{{ price.plans }} plans</strong>
            <span class="faint"> · about {{ perPlan(price) }} NIM each</span>
          </p>
        </div>
        ...
        <dl class="rows">
          ...
          <div class="rows__row">
            <dt>Network fee</dt>
            <dd>None — Nimiq Pay covers it</dd>
          </div>
        </dl>
      </template>
      ...
      <p class="fine faint">
        Credits are held against your wallet address. Nothing is stored about you, and payment goes
        straight from your wallet — Cairn never holds your funds.
      </p>
    </div>
  </div>
</template>
